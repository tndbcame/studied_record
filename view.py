...
class View(tk.Frame):
    ...
    def getFrame(self):
        ...
        self.show_studied_record_frame.pack(
            side=tk.BOTTOM, padx=50, pady=(0, 25))
        ...
        study_record_item.grid(
            column=0, row=1, sticky=tk.W, ipadx=15, ipady=15)
        study_record_timer_title.grid(
            column=0, row=2, sticky=tk.W, padx=15, pady=(20, 25))
    ...
    def show_query(self):
        ...
        self.canvas.config(scrollregion=self.canvas.bbox(
            "all"), highlightbackground="#355e88")

    def update_time(self):
        """時間更新関数"""
        self.after_id = self.input_studied_fram.after(
            self.INTERVAL, self.update_time)
        now_time = time.time()
        elapsed_time = now_time - self.start_time
        m, second = divmod(elapsed_time, 60)
        hour, minute = divmod(m, 60)
        self.timerView.config(
            text="{0:0=2g}:{1:0=2g}:{2:0=2.0f}".format(hour, minute, second))
